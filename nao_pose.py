...
from NaoSelfPose import NaoSelfPose


def loading(joint_names):
    # 读取机器人关节、连杆参数
    df_joints = pd.read_csv('nao_joints.csv')
    df_links = pd.read_csv('nao_links.csv')
    ...
    return joint_axis, joint_offsets


p = NaoSelfPose(loading)
print('!!!')

print(p.getJointPoses({'RHipRoll': 0.1}))
